# Lab 02: Build Your First Cowork Skill

In this lab you learn how to build custom Agent Skills for Copilot Cowork, how to manage skills in the product, and how to publish your own skills.
...
- Create skills manually at a lower level (for example with VS Code), package them, and upload them

## Exercise 1: Understand what Agent Skills are

In this exercise you build a clear mental model of skills, where they fit in Cowork orchestration, and how they differ from plugins.
...
Unlike one-off prompts, skills are durable assets you reuse across many conversations.

### Step 2: Compare built-in skills and custom skills

Cowork already includes built-in skills for common tasks like documents, communication, scheduling, and enterprise search. Create custom skills when:
...
Custom skills complement built-in skills. They do not replace all of Cowork behavior, they extend Cowork with your business context.
...
Choose a skill-first approach when your main goal is guided task execution. Add plugins and connectors when the workflow also needs external systems.

## Exercise 2: Manage skills in the Customize panel

In this exercise you explore how to manage available skills directly in Cowork.

### Step 1: Open the Customize experience

Open [Microsoft 365 Copilot](https://m365.cloud.microsoft), switch to 1️⃣ **Cowork**, and select 2️⃣ **Customize** in the left navigation.
...
![Customize screen in Copilot Cowork showing the Skills tab with user and built-in skills, highlighted by numbered callouts.](./_images/cowork-skills-01.png)
...
For your own skills, if any, validate that you can perform management operations such as:
...
![UI screen showing the copilot-camp-flash-cards skill with description, usage guidelines, and instructions for generating PowerPoint flash cards from Copilot Dev Camp labs.](./_images/cowork-skills-02.png)

After any edit, start a new conversation to test the behavior change.

## Exercise 3: Create a skill with the native Add skill flow

In this exercise you use Cowork's built-in guided authoring experience from **Customize** > **Skills**.

### Step 1: Start guided creation from Customize

In **Customize** > **Skills**, select **Add** > **Create new**.

![Copilot Customize screen showing the Skills tab, with a dropdown menu highlighting the Create new and Upload skill options.](./_images/cowork-custom-skill-01.png)

> **Important:** authoring and testing a custom skill in Copilot Cowork consumes Copilot Credits.

Cowork opens a guided conversation to collect your skill definition. Notice that Cowork is now using a native skill named **Skill management** to guide you through the creation of a new skill.

Cowork then prompts you to select the purpose of your skill. The available options are:

- Writing & drafting: generates recurring written content in your voice and format.
- Summarizing & briefing: condenses meetings, email threads, documents, or channels into a consistent structured summary.
- Data & analysis: pulls and formats data into a standard layout such as trackers, dashboards, or recurring metric reports.
- Process automation: runs a multi-step routine you do often, for example triage inbox, prep for a meeting, end-of-day wrap-up.
- Describe another option: freely describe your goal.

![UI showing the Copilot skill creation workflow with a Skill purpose dialog listing options and a sidebar highlighting Skills & Plugins with Skill Management.](./_images/cowork-custom-skill-02.png)

Select **Skip** to provide free text instructions for your custom skill.

For example, use the following text:

```text
Generate a set of flash cards in PowerPoint to test my knowledge about a specific lab of the Copilot Dev Camp.

Trigger this skill whenever the prompt includes "Create flash cards for a Copilot Dev Camp lab" or something similar, but still referring to "flash cards" and "Copilot Dev Camp".
...
Cowork starts processing your request: it checks the skill name, checks the capacity of your Cowork profile, generates the skill, stores it in your OneDrive for Business, validates it, and produces a quality report.

> You can have up to 50 custom skills configured in your profile, so when you create a new skill Cowork checks whether you are at your capacity limit.

You can test your skill directly from Cowork and iterate to fine-tune it. For example, provide the following prompt and check the result:
...
Once you are happy with your skill, close the current session. Cowork stored the skill in your OneDrive for Business in a folder with the following name:

```text
/Documents/Cowork/skills/<name-of-the-skill>
```

Browse your OneDrive for Business and check the content of the new skill folder.

![The folder on OneDrive for Business under Documents/Cowork/skills containing the SKILL.md definition file and the skill quality report JSON file.](./_images/cowork-custom-skill-03.png)

Start a new Cowork task and test a prompt that should trigger your new skill, for example:
...
In the right side panel, open **Skills** and verify that your custom skill appears among the active skills used during execution.
...
In this exercise you build a skill at a lower level by creating the `SKILL.md` file yourself. Alternatively you can download one of the many skills already available on the internet and upload it to Cowork, for example from [Skills.sh](https://www.skills.sh/).

### Step 1: Create the skill folder and author the SKILL.md file

Create a new folder on your file system, for example `weekly-status-mail`, and open it with Visual Studio Code. Add an empty `SKILL.md` file to the folder using the VS Code file explorer.
...
You can download the whole source of the skill from [the Copilot Dev Camp repository](https://github.com/microsoft/copilot-camp/blob/main/src/cowork/weekly-status-mail/SKILL.md).

Important checks:

- Keep `name` in kebab-case
- Ensure `name` aligns with the skill folder identity
- Keep the description explicit about trigger scenarios
...
After upload, verify that the included skills appear in **Your skills**.

If validation fails, check the common issues:
...
Start a new conversation and run a prompt that should activate your uploaded skill, for example:

```text
Draft my weekly status email
```

Verify activation from the side panel and confirm that outputs follow the expected workflow and format.

![The Cowork UI processing a task that triggers the new weekly-status-mail skill. The side panel shows the activated skill.](./_images/cowork-custom-skill-04.png)

## Summary

You created a Cowork skill twice: once through the guided **Add skill** flow, and once by hand-authoring `SKILL.md` and uploading it. You also know where Cowork stores your skills and how to verify activation.

Continue with [Lab 03: Build your first Cowork plugin](./lab-03-cowork-plugins.md).
