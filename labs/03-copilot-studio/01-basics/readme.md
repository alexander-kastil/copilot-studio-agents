# Copilot Studio Basics

- Overview Copilot Studio Agents in Copilot Studio
- PowerPlatform Admin Center & Architecture
- Creating & Configuring Copilot Studio Agents
- Mastering Knowledge & Starter Prompts
- Topics, Entities and Variables
- Introduction to Power FX
- Conversation Design Best Practices
- Collecting Information using Adaptive Cards
- Publish Agents to Channels (Teams, Charts)

## Labs

### Lab 0: Activate Copilot Studio

[Task 1: Create a Power Platform environment and activate Copilot Studio](./lab-01-setup-environment.md)

### Lab 1: Create your first Copilot Studio Agent

- [Task 1: Create your first Declarative Agent](https://learn.microsoft.com/en-us/training/modules/build-your-first-agent-microsoft-365-copilot-use-copilot-studio/3-exercise-create-declarative-agent)

- [Task 2: Add custom knowledge to your Agent](https://learn.microsoft.com/en-us/training/modules/build-your-first-agent-microsoft-365-copilot-use-copilot-studio/5-exercise-custom-knowledge)

- [Task 3: Add suggested prompts](https://learn.microsoft.com/en-us/training/modules/build-your-first-agent-microsoft-365-copilot-use-copilot-studio/7-exercise-starter-prompts)

- [Task 4: Add a Prompt Tool to your Agent](https://learn.microsoft.com/en-us/training/modules/extend-declarative-agents-prompt-actions-copilot-studio/4-exercise-create-prompt-action)

### Lab 2: Conversation Design and Topics

- [Task 1: Create a base Agent](./lab-02-first-agent.md)

## Lab 3: Power Fx

- [Task 1: Work with Power Fx functions](https://learn.microsoft.com/en-gb/training/paths/work-powerfx-functions/)

## Lab 4 Adaptive Cards

- [Task 1: Create and test Adaptive Cards with the online designer](https://learn.microsoft.com/en-us/training/modules/adaptive-cards-create-engaging-messages/3-exercise-online-designer)

- [Task 2: Define Topics and use Adaptive Cards in Copilot Studio](./lab-03-topics.md)
