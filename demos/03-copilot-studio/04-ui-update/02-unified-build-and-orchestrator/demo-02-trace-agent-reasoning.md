...
This demo puts [The Unified Build Surface and the New Orchestrator](readme.md) into practice by exercising the single enhanced runtime that plans and reasons across multiple steps, then exposing that plan through Get rationale and the activity tracker. It builds on [The Two Copilot Studio Experiences](../01-new-experience-overview/readme.md), because only new-type agents run this one non-configurable orchestration runtime; a classic agent would answer single-pass with one tool per turn and would not expose a chained plan.
...
- The Northwind Sales Assistant you built in [Build and Ground Your First New-Experience Agent](../01-new-experience-overview/demo-01-build-and-ground-an-agent.md), grounded on the product and account reference document, open on the **Build** tab. This demo continues that same agent; do not start a new one.
...
| Tabs read Topics / Knowledge / Actions / Settings | The agent is a classic-experience agent | This flow needs a new-type agent; return to [Build and Ground Your First New-Experience Agent](../01-new-experience-overview/demo-01-build-and-ground-an-agent.md) and use that agent |
