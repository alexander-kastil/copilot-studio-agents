...
This demo uses the Daily Briefing, Enterprise Search, and Calendar Management capabilities from the built-in [Skills](../02-built-in-skills/readme.md) catalogue to compress a backlog into a plan. It is a concrete run of the [Getting Started with Cowork](./readme.md) interaction model, and the drafted replies stop at the same approval gate described in [Human-in-the-Loop](../06-staying-in-control/readme.md).
...
Point Cowork at the `demo-02-monday-briefing` folder.
...
In Microsoft 365 Copilot Cowork, start a new task, click **+**, and attach `inbox-snapshot.xlsx` from the `demo-02-monday-briefing` folder. Keep it attached throughout.
...
> How the approval gate and per-action risk levels work is covered in [Human-in-the-Loop](../06-staying-in-control/readme.md).
