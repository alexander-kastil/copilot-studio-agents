...
This demo shows the [Getting Started with Cowork](../01-intro/readme.md) promise of end-to-end work: one request that spans reading, editing, emailing, and scheduling. The document and calendar work draws on built-in [Skills](../02-built-in-skills/readme.md), and because the final email reaches an external partner, it exercises the high-risk approval path from [Human-in-the-Loop](./readme.md).
...
In Microsoft 365 Copilot Cowork, start a new task, click **+**, and attach both files from the `demo-01-follow-up` folder. They remain available for every step.
...
> Why sending an email is high risk while scheduling is medium is explained in [Human-in-the-Loop](./readme.md#risk-levels).
